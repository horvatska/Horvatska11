...
if (iconSrc) {
  ...
  createTaskbarIcon(id, "assets/images/win.png", "O sustavu");
...
function closeWindow(id) {
  ...
  // When an IE window is closed, remove the active class from the pinned button
  document.getElementById("ie-button").classList.remove("active");
}


document.getElementById("ie-button").addEventListener("click", () => {
  // First, check if an Internet Explorer window is already open
  const existingWindow = Array.from(document.querySelectorAll(".window")).find(w => {
    const title = w.querySelector(".title")?.textContent || "";
    return title === "Internet Explorer";
  });

  if (existingWindow) {
    // If it exists, just bring it to the front and ensure the icon is active
    existingWindow.style.zIndex = zIndexCounter++;
    document.getElementById("ie-button").classList.add("active");
  } else {
    // If it doesn't exist, open a new one directly, skipping the warning.
...
function openBrowserWindow(url) {
  ...
  // Call createWindow but pass null for iconSrc to prevent a new icon from being made
  createWindow(
    url,
    "Internet Explorer",
    browserId,
    null, // This is the key change: DO NOT pass an icon source
    "Internet Explorer"
  );
  // Add the 'active' class to the pinned button instead
  document.getElementById("ie-button").classList.add("active");
}
...
// Unified function to handle closing the start menu with animation
function closeStartMenu() {
  if (!startMenu.classList.contains("open")) return;

  // Remove .open to trigger the slide-down animation
  startMenu.classList.remove("open");

  // Use a timeout that matches your CSS transition duration to hide it after it slides down
  // This is more reliable than 'transitionend' if multiple properties are transitioning.
  setTimeout(() => {
    startMenu.classList.add("hidden");
  }, 160); // Match this duration (in ms) to your CSS transition
}
...
startButton.addEventListener("click", (e) => {
  ...
  if (isOpen) {
    ...
    // Open: make it visible, force the browser to repaint, then add .open to trigger the slide-up animation.
    startMenu.classList.remove("hidden");
    void startMenu.offsetHeight; // This forces the browser to apply the 'hidden' removal before the next step.
    startMenu.classList.add("open");
  }
});

// close start menu when clicking anywhere else on the page
